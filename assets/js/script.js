document.addEventListener("DOMContentLoaded", () => {
    ...
    // Reveal all bombs on the board (called when game is over)
    function revealBombs() {
        squares.forEach((sq) => {
            if (sq.classList.contains("bomb")) {
                sq.classList.add("checked");
                // Use bomb emoji to show bombs
                sq.textContent = "💣";
            }
        });
    }
    ...
    function checkForWin() {
        ...
        // Count all valid (non-bomb) squares
        const totalValid = squares.filter(sq => sq.classList.contains("valid")).length;
        // Count all valid squares that have been revealed
        const checkedValid = squares.filter(
            sq => sq.classList.contains("valid") && sq.classList.contains("checked")
        ).length;

        // Win condition: all non-bomb squares are revealed
    ...
    // Handle game over when a bomb is clicked
    function gameOver() {
        isGameOver = true;      // Mark game as over
        stopTimer();            // Stop the timer
        alert("Game Over! You hit a bomb."); // Show game-over message
        revealBombs();          // Reveal all bombs on the board
    }

    // Add or remove a flag on right-click
    function addFlag(square) {
        // If game is over, don't allow flagging
        if (isGameOver) return;
        // Only allow flags on unchecked squares and if we have flags left
        if (!square.classList.contains("checked") && flags < bombsAmount) {
            if (!square.classList.contains("flag")) {
                ...
                square.innerHTML = "🚩";
                flags++;
                updateMinesCount(); // Update remaining mines display
            } else {
                // Remove flag
    ...
    // Reset the entire game (board, timer, flags)
    function resetGame() {
    ...
    // Attach reset action to reset button if it exists
    resetButton?.addEventListener("click", resetGame);

    // Create the initial board when the page loads
    createBoard();
});
